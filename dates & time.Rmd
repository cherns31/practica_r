...
flights_dt %>% 
  ...
  ggplot(aes(dep_time)) + 
  geom_freqpoly(binwidth = 1800) # 1800 s = 30 minutes
```
...
```{r}
(one_pm <- ymd_hms("2016-03-13 13:00:00", tz = "America/New_York"))
one_pm + ddays(1) # Cause it's DSL time
...
Unless otherwise specified, lubridate always uses UTC. UTC (Coordinated Universal Time) is the standard time zone used by the scientific community and roughly equivalent to its predecessor GMT (Greenwich Mean Time). It does not have DST, which makes a convenient representation for computation.
...
```{r}
x4b <- force_tz(x4, tzone = "America/Buenos_Aires")
x4b
x4b - x4
```
