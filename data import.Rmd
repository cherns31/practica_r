...
```{r parse integer with na}
parse_integer(c("1", "231", ".", "456"), na = ".")
```

If parsing fails, you’ll get a warning. If there are many parsing failures, you’ll need to use problems() to get the complete set. This returns a tibble, which you can then manipulate with dplyr.
...
```{r locale months}
parse_date("15 agosto de 2019", "%d %B %* %Y", locale = locale("es"))
```
...
```{r readr example parsing problems}
...
```{r}
read_lines(readr_example("challenge.csv"), n_max = 20)
...
```{r write to feather format, warning=F}
library(feather)
